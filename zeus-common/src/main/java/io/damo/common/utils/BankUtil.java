...
/**
 * 根据卡号 查出对应的银行卡名称， 准确性需要测试
 *
 */
public class BankUtil {
    private static Map<String, String> BankName2BankCodeMap = new HashMap<>();
    private static Map<String, String> cardPrefix2TypeMap = A.newHashMap();

    //后期需要不断添加这里支持的银行
    static {
        BankName2BankCodeMap.put("中国建设银行","CCB");
        BankName2BankCodeMap.put("建设银行","CCB");
        BankName2BankCodeMap.put("中国农业银行","ABC");
        BankName2BankCodeMap.put("中国工商银行","ICBC");
        BankName2BankCodeMap.put("工商银行","ICBC");
        BankName2BankCodeMap.put("中国银行","BOC");
        BankName2BankCodeMap.put("中国民生银行","CMBC");
        BankName2BankCodeMap.put("民生银行","CMBC");
        BankName2BankCodeMap.put("招商银行","CMB");

        readCardBinExcel(BankUtil.class.getResourceAsStream("/2016年12月10日版卡表.xls"));
    }

    public static boolean isValidCardNo(String cardNo) {
        return U.isNotBlank(cardNo) && cardNo.length() >= 16 && cardNo.length() <= 19;
    }

    //中国银联会每月发布2次 卡表信息，从表中可以根据卡号前6位 来判断是 借记卡还是银行卡
    public static String getCardType(String cardNo) {
        String prefix = "";
        for (String p : cardPrefix2TypeMap.keySet()) {
        ...
        return cardPrefix2TypeMap.get(prefix);
    }

    public static boolean isCreditCard(String cardNo) {
        String cardType = cardPrefix2TypeMap.get(cardNo.substring(0,6));
        return U.isNotBlank(cardType) && cardType.equals("贷记卡");
    }

    //获取银行卡所属的银行的代码
    public static String getBankCodeOfCard(String idCard) {
        String bankCode = BankName2BankCodeMap.get(getBankNameOfCard(idCard));

        return bankCode != null ? bankCode : "未知银行";
    }

    public static String getBankNameOfCard(String bankCardNo) {
        String bankName = getNameOfCard(bankCardNo).split("·")[0];
        return bankName;
    }

    // 传入卡号 得到银行卡名称
    public static String getNameOfCard(String idCard) {
        int index = -1;
        if (idCard == null || idCard.length() < 16 || idCard.length() > 19) {
            return "无效银行卡号";
        }

        // 4位Bin号 -- 新的卡都是6位，但是旧的卡，比如9558开头的工行卡
        String cardbin_4 = idCard.substring(0, 4);
        for (int i = 0; i < bankBin.length; i++) {
            if (cardbin_4.equals(bankBin[i])) {
                index = i;
            }
        }
        if (index != -1) {
    ...
    //读取银联的银行卡表信息
    //如果 有 Duplicate name " DocumentSummaryInformation错误，重新保存下xls文件即可
    public static void readCardBinExcel(InputStream is) {
        try (POIFSFileSystem fs = new POIFSFileSystem(is);
             HSSFWorkbook wb = new HSSFWorkbook(fs)
        )  {

            Sheet sheet = wb.getSheetAt(0);
            int rowIndex = 4; //从第5行开始是卡表信息
            ...
            while (row != null) {
                cardPrefix = row.getCell(13).getStringCellValue();
                cardType = row.getCell(15).getStringCellValue();
                cardPrefix2TypeMap.put(cardPrefix, cardType);
    ...
    private final static String[] bankBin = {"621098", "622150", "622151",
            ...
            "621481", "621310", "621396", "623251", "628351", "9558"};
    // "发卡行.卡种名称",
