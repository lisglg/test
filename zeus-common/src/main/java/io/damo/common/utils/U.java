...
import io.damo.common.exception.ServiceMustHandleException;
...
public final class U {
    ...
    public static boolean checkRegexWithStrict(String param, String regex) {
        return isNotBlank(param) && compile(regex).matcher(param).matches();
    }
    ...
    public static boolean checkRegexWithRelax(String param, String regex) {
        return isNotBlank(param) && compile(regex).matcher(param).find();
    }
    ...
    public static String getField(Object data, String field) {
        ...
        if (value == null) {
            ...
            // 如果是日期, 则格式化
            return getNil(DateUtils.formatFull((Date) value));
        } else {
            return getNil(value);
        }
    }
    ...
    public static String getStringRandom(int length) {
        ...
        for (int i = 0; i < length; ++i) {
            ...
            st = sb.append(str.charAt(number)).toString();
        }
        Boolean flag = sb.toString().matches("[A-Z]+");
        if (flag) {
            st = getStringRandom(5);
        }
        return st;
    }
